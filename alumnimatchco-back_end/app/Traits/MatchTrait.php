...
trait MatchTrait
{
    ...
    private function getCLMatchRatio($uid1, $uid2) {
        ...
        $sum += $this->getHealthMatchRatio($uid1, $uid2);
    ...
    private function getHealthMatchRatio($uid1, $uid2) {
        $health1 = DB::table('user_healths')->where('uid', $uid1)->first();
        $health2 = DB::table('user_healths')->where('uid', $uid2)->first();
        if (is_null($health1) || is_null($health2)) {
            return 0;
        } else {
            $match_ratio = 0;
            if (!is_null($health1->mental) && !is_null($health2->mental) && $health1->mental === $health2->mental) {
                $match_ratio += 0.5;
            }
            if (!is_null($health1->physical) && !is_null($health2->physical) && $health1->physical === $health2->physical) {
                $match_ratio += 0.5;
            }
            return $match_ratio;
        }
