...
class UserController extends Controller
{
    ...
    public function index(Request $request) {
        $user = User::where('id', $request->get('user')->id)->withCount([
            'friends',
            'visits',
        ...
        $cl = [
            ...
            'health' => DB::table('user_healths')->where('uid', $user->id)->first(),
    ...
    public function saveGenderAgeEthnicity(Request $request) {
        ...
        DB::table('user_gender_ages')->updateOrInsert(
            ...
            [
                ...
                'age' => isset($request['gender']) ? $request['gender'] : null,
            ]
        );
    ...
    public function saveRelationshipDivorced(Request $request) {
        ...
        DB::table('user_relationship_single')->updateOrInsert(
            ...
            [
                ...
                'ethnicity' => $request->has('ethnicity') ? $request['ethnicity'] : null,
                'music' => $request->has('music') ? $request['music'] : null,
                ...
                'match_age' => $request->has('match_age') ? $request['match_age'] : null,
                'bodytype' => $request->has('bodytype') ? $request['bodytype'] : null,
...
//        TODO: Send email invitation to partner
    ...
    public function saveWorkCareer(Request $request) {
        ...
        DB::table('user_work_careers')->updateOrInsert(
            ...
            [
                ...
                'buying_stuff' => $request->has('own_business') && $request['own_business'] === 0 && $request->has('buying_stuff') ? $request['buying_stuff'] : null,
                'customer' => $request->has('own_business') && $request['own_business'] === 0 && $request->has('customer') ? $request['customer'] : null,
    ...
    public function saveHealth(Request $request) {
        $user = $request->get('user');
        DB::table('user_healths')->updateOrInsert(
            [
                'uid' => $user->id
            ],
            [
                'mental' => $request->has('mental') ? $request['mental'] : null,
                'mental_privacy' => $request->has('mental_privacy') ? $request['mental_privacy'] : null,
                'physical' => $request->has('physical') ? $request['physical'] : null,
                'physical_privacy' => $request->has('physical_privacy') ? $request['physical_privacy'] : null
            ]
        );
        return response()->json(true);
    }
    ...
    public function getCLProfileCompleted(Request $request) {
        ...
        if (DB::table('user_healths')->where('uid', $user->id)->count() > 0) {
            $completes['health'] = true;
        } else {
            $completes['health'] = false;
        }
    ...
    public function getProfileCompleted(Request $request) {
        ...
        if (DB::table('user_healths')->where('uid', $user->id)->count() > 0) {
            ...
            array_push($uncompleted_items, 'Health');
        }
        if (DB::table('user_hobbies')->where('uid', $user->id)->count() > 0) {
            $count++;
        } else {
    ...
    private function buildInviteCode($uid) {
        ...
        return [
            'code' => $six_digit_random_number,
            'expired' => Carbon::parse(Carbon::now())->add('minute', 15)
        ];
    }
